...
def GAN_loss(Z, X, generator_settings, discriminator_settings,cond, CG, CD, CS, wrong_labels=False):
    ...
    temp = tf.floor(tf.scalar_mul(2,D_real[:,seq_length-1,0]))
    count_real = tf.reduce_sum(temp)

    temp = tf.floor(tf.scalar_mul(2,D_fake[:,seq_length-1,0]))  
    count_fake = bsize - tf.reduce_sum(temp)
